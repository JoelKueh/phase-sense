...
void CL_CALLBACK nbody_err_cb(const char *error_info, const void *private_info,
...
int nbody_ctx_init(nbody_context_t *ctx)
{
    ...
    ctx->cl_q = clCreateCommandQueueWithProperties(ctx->cl_ctx, device_ids[0], q_props, &errcode);
    CL_ERR_PANIC(errcode);
...
int nbody_sim_init(nbody_context_t *ctx, int vbo, int ppv, int n)
{
    ...
    ctx->d_accel = clCreateBuffer(ctx->cl_ctx, CL_MEM_READ_WRITE,
								  sizeof(float) * 2 * n, NULL, &errcode);
    CL_ERR_PANIC(errcode);
    ctx->d_coll = clCreateBuffer(ctx->cl_ctx, CL_MEM_READ_WRITE, sizeof(float) * n,
                                 NULL, &errcode);
    CL_ERR_PANIC(errcode);
    ctx->d_sync_flag = clCreateBuffer(ctx->cl_ctx, CL_MEM_READ_WRITE, sizeof(int),
                                      NULL, &errcode);
    CL_ERR_PANIC(errcode);
    ...
    CL_ERR_PANIC(clEnqueueFillBuffer(ctx->cl_q, ctx->d_accel, &fill_pattern, sizeof(float),
...
void nbody_update(nbody_context_t *ctx, float dt)
{
	...
	CL_ERR_PANIC(clEnqueueNDRangeKernel(ctx->cl_q, ctx->k_collisions, 1, NULL,
	...
	CL_ERR_PANIC(clEnqueueNDRangeKernel(ctx->cl_q, ctx->k_sync_clusters, 1, NULL,
	                       &global_size, &local_size, 0, NULL, NULL));
	...
	CL_ERR_PANIC(clEnqueueNDRangeKernel(ctx->cl_q, ctx->k_accel_walk, 1, NULL,
	                       &global_size, &local_size, 0, NULL, NULL));
	...
	CL_ERR_PANIC(clEnqueueNDRangeKernel(ctx->cl_q, ctx->k_update, 1, NULL,
	                       &global_size, &local_size, 0, NULL, NULL));
